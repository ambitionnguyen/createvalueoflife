	...
	"use strict";
		...
		$(window).load(function() {	
			if( $(window).width() > 1024 ){ //window width bigger than 1024
				...
				var sticky_wrapper = $('<div class="sticky_nav_wrap"></div>');
...
(function($){
	...
	$.fn.rt_waypoint = function()
	{ 
		var i = 1; 

		$(this).find('[data-rt-animate="animate"]').each(function(i){
			$(this).waypoint( { 
				triggerOnce: true,   
				offset: function() { 
	...
	if( rt_theme_params["content_animations"] && ! $("body").hasClass("mobile_device") ){		
		...
		$("body").addClass("rt_content_animations");

		/* start scroll to load items */
		$(window).load(function() {	
			if ( $.fn.waypoint ) {  
				$('.row,#header_contents,.top_content').each(function(i){			
					$(this).rt_waypoint();
				});		
			}
		});
		...
		/* 
			set min-height for layer slider  
