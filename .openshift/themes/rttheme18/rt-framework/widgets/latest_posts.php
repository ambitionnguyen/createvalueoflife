	...
	function widget( $args, $instance ) {
		...
		if ($post_query->have_posts()) : while ($post_query->have_posts()) : $post_query->the_post();							
			...
			if ( $thumbnail_image_output && ! $show_thumbnails ) : 
				$rt_posts .= "<a href='".esc_url( get_permalink() )."'>".$thumbnail_image_output."</a>";
			endif;		
			...
			$rt_posts .= ($show_excerpt) ? ''.wp_html_excerpt(get_the_excerpt(),$limit).'...'.'<br/>&nbsp;&nbsp;&nbsp;<a class="readmore" href="'. esc_url( get_permalink() ) . '">' . 'Read more ...'. '</a>'.'<div class="space margin-b10"></div>' : "" ;
