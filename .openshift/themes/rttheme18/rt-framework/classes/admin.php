...
class RTThemeAdmin extends RTTheme{
	...
	function icon_selection() {  
		...
		$fontawesome_json_file =  "/css/fontello/config_more.json";
		...
		$fontawesome_json_url = rt_locate_media_file( $fontawesome_json_file ) ; 

		//try with wp_remote_fopen first
		$json = wp_remote_fopen( $fontello_json_url ); 
		$json_awesome = wp_remote_fopen( $fontawesome_json_url ); 
		...
		//try to include if no json returned
		if ( ! json_decode($json_awesome) ){
			ob_start(); 

			if( file_exists( get_stylesheet_directory(). $fontawesome_json_url ) ){
				include( get_stylesheet_directory(). $fontawesome_json_url ); 
			}else{
				include( get_template_directory() . $fontawesome_json_url  ); 
			}
				
			$json_awesome = ''.ob_get_contents().'';
			ob_end_clean(); 
		}

		//paste the list output
		if ( $json_awesome ){
			$json_output = json_decode($json_awesome);
