...
#
# Load header widget area Custom
#
# @filters = sidebar-for-top-second - sidebar-for-top-first
#
if( ! function_exists("rt_header_widgets_second_custom") ){
    function rt_header_widgets_second_custom($widget_area, $class_name){
        echo '<section class="section_widget '. $class_name .'">';
        echo '<span class="wrap-member-login"><a id="member-login" href="https://trp.customerhub.net">Member login</a></span>';
...
if( ! function_exists("rt_header_output_function") ){
	function rt_header_output_function(){
		...
		if( $show_second_top_widget && $logo_position == "right" ){
			rt_header_widgets_second_custom( $second_top_widget_name, "second ".$widget_class_name );
		}
	...
	function rt_top_slogan(){
		...
		$add_class = $rt_header_layout_options["logo_position"] == "left" || $rt_header_layout_options["logo_position"] == "center" ? "right_side " : "left_side";
                ...
                if( get_option(RT_THEMESLUG.'_social_media_top') ){
                    $social_media = do_shortcode("[rt_social_media_icons]");
                }
                else{
                    $social_media = '';
                }
		...
		$output= sprintf('
			<section id="slogan_text" class="'. $add_class  .'">
				%s %s
			</section>%s',$top_slogan_icon,$top_slogan,$social_media);
...
if (! function_exists("mag_social_share")) {

	function mag_social_share() {
		echo '<ul class="social_media">';
		// echo '<li class="mail">';
		// echo '<a class="icon-mail" href="mailto:?subject='.urlencode(get_the_title()).'&amp;body='.urlencode(get_the_excerpt()).'." title="Email"><span>Email</span></a>';
		// echo '</li>';
		echo '<li class="facebook">';
		echo '<a class="icon-facebook" href="'.get_permalink().'" title="Facebook"><span>Facebook</span></a>';
		echo '</li>';
		echo '<li class="gplus">';
		echo '<a class="icon-gplus" href="https://plus.google.com/share?url='.urlencode(get_permalink()).'" title="Google +"><span>Google +</span></a>';
		echo '</li>';
		echo '<li class="linkedin">';
		echo '<a class="icon-linkedin" href="http://www.linkedin.com/shareArticle?mini=true&url='.urlencode(get_permalink()).'" title="Linkedin"><span>Linkedin</span></a>';
		echo '</li>';
		echo '</ul>';
	}
}
...
if( ! function_exists("rt_post_meta") ){
	...
	?>
		...
		<div class="post_data">
            ...
            <div class='share_blog'><span>SHARE</span> <?php mag_social_share() ?></div>
...
if( ! function_exists("rt_header_bar_output_function") ){
	...
	function rt_header_bar_output_function(){
		...
		//TrungPham - Add header text for Coaching + Consulting + Speaking + Success Page 
		if (str_replace("templateid_", "", $rt_templateID ) == 568909){
			echo '<section class="info_bar clearfix "><section class="heading"><h1>Coaching</h1></section> </section>';	
		} elseif (str_replace("templateid_", "", $rt_templateID ) == 184832) {
			echo '<section class="info_bar clearfix "><section class="heading"><h1>Consulting</h1></section> </section>';	
		} elseif (str_replace("templateid_", "", $rt_templateID ) == 650089) {
			echo '<section class="info_bar clearfix "><section class="heading"><h1>Speaking</h1></section> </section>';	
		} elseif (str_replace("templateid_", "", $rt_templateID ) == 768920) {
			echo '<section class="info_bar clearfix "><section class="heading"><h1>Success</h1></section> </section>';	
		}
	...
	function rt_create_photo_gallery( $atts ){
		...
		foreach ($image_array as $image) { 								 
			...
			$items_output .= sprintf('
				<li class="box %s">
					<div class="%s" data-rt-animate="animate" data-rt-animation-type="flipInX" data-rt-animation-group="product_carousel">	
						%s							
						<img src="%s" alt="%s"> 									
					</div>
					%s 	
		...
		$gallery_holder_output = sprintf('
			<div class="row clearfix">
				<ul class="photo_gallery" id="%s">%s</ul> 
			</div>
