...
if ( ! function_exists( 'presscore_get_dynamic_stylesheets_list' ) ) :
	...
	function presscore_get_dynamic_stylesheets_list() {
		...
		if ( null === $dynamic_stylesheets ) {
			...
			$file_path =  $template_directory . '/css/custom.less';
			if(file_exists( $template_directory . '\css\custom.less')){
				$file_path =  $template_directory . '\css\custom.less';
			}
...
if ( ! function_exists('add_request_detail') ) {
	function add_request_detail( $components, $wpcf7 )
	{
		...
		$indexPick = 0;
		while (isset($_POST['pickCity'.$indexPick])) {
			$pickContent .= '<p style="margin: 15px 0 10px 0;">';
			$pickContent .= '<ul>';
			$pickContent .= '<li>City: '.$_POST['pickCity'.$indexPick].'</li>';
			$pickContent .= '<li>State: '.$_POST['pickState'.$indexPick].'</li>';
			$pickContent .= '<li>Zip/Postal Code: '.$_POST['pickZipcode'.$indexPick].'</li>';
			$pickContent .= '</ul>';
			$pickContent .= '</p>';
			$indexPick ++;
		}
		...
		$indexDelivery = 0;
		while (isset($_POST['deliveryCity'.$indexDelivery])) {
			$deliveryContent .= '<p style="margin: 15px 0 10px 0;">';
			$deliveryContent .= '<ul>';
			$deliveryContent .= '<li>City: '.$_POST['deliveryCity'.$indexDelivery].'</li>';
			$deliveryContent .= '<li>State: '.$_POST['deliveryState'.$indexDelivery].'</li>';
			$deliveryContent .= '<li>Zip/Postal Code: '.$_POST['deliveryZipcode'.$indexDelivery].'</li>';
			$deliveryContent .= '</ul>';
			$deliveryContent .= '</p>';
			$indexDelivery ++;
		}

		$body = preg_replace( '/{placeHolder4Pick}/', $pickContent, $body );
		$body = preg_replace( '/{placeHolder4Delivery}/', $deliveryContent, $body );
		// replace the body with our modified body
...
add_filter( 'wpcf7_mail_components', 'add_request_detail', 10, 2 );